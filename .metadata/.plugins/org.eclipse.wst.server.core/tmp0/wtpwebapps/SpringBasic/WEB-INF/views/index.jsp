...
<body>
	...
	<div id="wrapper">
		...
		<ul class="actions vertical">
			...
			<form action="member/join" method="post">
				...
				<li><input type="button" value="중복확인" onclick="checkEmail()"></li>
	...
	<script>
		function checkEmail(){
			...
			// innerHtml : 태그사이의 html 요소 포함, html()
			// html() -> 값 가지고 오는거, html("<h1>text</h1>") -> 값 추가하는거
			// innerText : 태그사이의 텍스트만
			// 텍스트
			// <input>
			// value : input 태그에 작성된 값(사용자가 입력한(선택한 값)), val()
			let input = $("#input").val()
			console.log(input)
			...
			$.ajax({
				...
				success : function(res){
					console.log(res)
					if(res=='success'){
						$("#result").text('사용할 수 있는 이메일입니다.')
					} else { //fail
						$("#result").text('사용할 수 없는 이메일입니다.')
					}
				},
				error : function(){
					alert("통신실패")
